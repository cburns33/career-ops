 ...
 *
 * Data flow:
 *   portals.yml ΓåÆ scanner.parseConfig() ΓåÆ scanner.scan() ΓåÆ listings + skipped
 *   ΓåÆ saveJd() writes jds/*.md
 *   ΓåÆ appendToPipeline() writes data/pipeline.md
 *   ΓåÆ appendScanHistory() writes data/scan-history.tsv
 *
...
// ---------------------------------------------------------------------------
// Scanner registry ΓÇö add new portal scanners here
// ---------------------------------------------------------------------------
...
function warn(msg) { console.warn(`[scan-auth] ΓÜá ${msg}`); }
function error(msg) { console.error(`[scan-auth] Γ£ù ${msg}`); }
...
function appendToPipeline(listings) {
  ...
  if (!existsSync(PIPELINE_PATH)) {
    warn(`pipeline.md not found ΓÇö listings not added. Create data/pipeline.md or run onboarding first.`);
    return;
  }
  ...
  const block = listings.map(l =>
    `- [ ] ${l.url} | ${l.company.replace(/\|/g, 'ΓÇö')} | ${l.title.replace(/\|/g, 'ΓÇö')}`
  ).join('\n') + '\n';
...
async function waitForLogin(page) {
  ...
  console.log(`Γòæ  Please log in to ${scanner.name.padEnd(10)} in the browser window Γòæ`);
  console.log('Γòæ  Press ENTER here once you\'re logged in...       Γòæ');
...
# ${detail.title} ΓÇö ${detail.company}
...
function printSummary(results) {
  ...
  console.log('\nΓòöΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòù');
  console.log(`Γòæ${' '.repeat(left + 1)}${label}${' '.repeat(right + 1)}Γòæ`);
  console.log('ΓòáΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòú');
  console.log(`Γòæ  Searches run:      ${String(s.searched).padStart(4)}                        Γòæ`);
  console.log(`Γòæ  Listings found:    ${String(s.found).padStart(4)}                        Γòæ`);
  console.log(`Γòæ  Extracted:         ${String(s.extracted).padStart(4)}                        Γòæ`);
  console.log(`Γòæ  Filtered out:      ${String(s.skipped_filter).padStart(4)}                        Γòæ`);
  console.log(`Γòæ  Already seen:      ${String(s.skipped_dedup).padStart(4)}                        Γòæ`);
  console.log(`Γòæ  Viewed skipped:    ${String(s.skipped_viewed ?? 0).padStart(4)}                        Γòæ`);
  console.log(`Γòæ  JDs saved:         ${String(s.saved).padStart(4)}                        Γòæ`);
  console.log(`Γòæ  Errors:            ${String(s.errors).padStart(4)}                        Γòæ`);
  console.log('ΓòÜΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓò¥');

  if (results.listings.length > 0) {
    console.log('\nNew listings:');
    for (const l of results.listings) {
      console.log(`  ΓÇó ${l.title} ΓÇö ${l.company}`);
    }
...
async function main() {
  ...
  try {
    ...
    if (loggedIn) {
      log('Session active ΓÇö logged in');
    } else {
      warn('Not logged in ΓÇö login required');
    }
    ...
    if (!loggedIn && FLAG.login) {
      log(`Login mode ΓÇö opening ${scanner.name} login page`);
    ...
    if (FLAG.login) {
      log(`Login successful ΓÇö ${scanner.name} session saved. Run again without --login to scan.`);
      return;
    }

    // Scanner handles extraction, filtering, dedup ΓÇö returns accepted listings
