 ...
 *   3. isJobCardViewed() checks for LinkedIn's "Viewed" label ΓÇö skips
 *      before clicking so viewed cards are never opened
 *   4. Blocklist, dedup (company::title), and title filter run against
 *      the preview ΓÇö dedup catches cross-portal matches
 ...
 *   - External apply link (via xpathApplyUrl) ΓåÆ unwrapped from LinkedIn's
 *     /safety/go redirect
 *   - Easy Apply / no external link ΓåÆ falls back to the listing URL
...
// ---------------------------------------------------------------------------
// Selectors ΓÇö grouped for easy maintenance when LinkedIn changes DOM
// ---------------------------------------------------------------------------
...
const SELECTORS = {
  ...
  viewedStatusLineSeparatorCharClass: '┬╖ΓÇó|',
};
...
function warn(msg) { console.warn(`[linkedin] ΓÜá ${msg}`); }
...
export default class LinkedInScanner {
  ...
  // -------------------------------------------------------------------------
  // Config parsing ΓÇö extracts linkedin_searches section from portals.yml
  // -------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  // Scan ΓÇö the main extraction loop
  //
  ...
  async scan(context, config, options = {}) {
    ...
    // Single page for all searches ΓÇö avoids Playwright stealing window focus
    const page = await context.newPage();

    for (const search of toRun) {
      log(`\nΓöÇΓöÇ Search: ${search.name} ΓöÇΓöÇ`);
      stats.searched++;
        ...
        while (hasNextPage && accepted < maxPerSearch) {
          ...
          for (let i = 0; i < cardCount; i++) {
            ...
            if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
              warn(`${MAX_CONSECUTIVE_FAILURES} consecutive extraction failures ΓÇö stopping this search (likely throttled or DOM changed)`);
              break;
            }
            ...
            if (!preview.title) {
              warn(`  Γ£ù No title on card ${i} (preview); skipping`);
              stats.errors++;
              consecutiveFailures++;
              continue;
            }
            ...
            if (employerBlocklist.length && preview.company) {
              ...
              if (employerBlocklist.some(b => companyLower === b.toLowerCase())) {
                log(`  Γ£ù Blocked employer: ${preview.company}`);
            ...
            if (companyTitleKey && scanHistory.has(companyTitleKey)) {
              log(`  Γ£ù Already seen: ${preview.title} (${preview.company})`);
              stats.skipped_dedup++;
              skipped.push({
                url: '',
                title: preview.title || '',
            ...
            if (!this.#matchesFilter(preview.title, '', titleFilter)) {
              log(`  Γ£ù Filtered: ${preview.title} (${preview.company})`);
              stats.skipped_filter++;
              continue;
            }

            // 4. All pre-click checks passed ΓÇö click card and extract detail
            ...
            if (data.jobId && scanHistory.has(data.jobId)) {
              log(`  Γ£ù Already seen: ${data.title} (${data.company})`);
              stats.skipped_dedup++;
              skipped.push({
                url: data.listingUrl || '',
                title: data.title || '',
            ...
            if (!data.applicationUrl) {
              log(`  Γ£ù No apply URL: ${data.title} (${data.company})`);
              stats.skipped_filter++;
              continue;
            }

            if (!this.#matchesFilter(data.title, data.jdText || '', titleFilter)) {
              log(`  Γ£ù Filtered after JD: ${data.title} (${data.company})`);
              stats.skipped_filter++;
              continue;
            }

            if (!data.jdText) {
              warn(`  Γ£ù No JD content: ${data.title}`);
              stats.errors++;
              consecutiveFailures++;
              continue;
            }
            ...
            accepted++;
            log(`  Γ£ô Accepted: ${data.title} at ${data.company}`);
            } finally {
              await card.dispose();
            }
          }
  ...
  // -------------------------------------------------------------------------
  // Private ΓÇö search URL construction
  // -------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  // Private ΓÇö extraction helpers
  // -------------------------------------------------------------------------
  ...
  async isJobCardViewed(page, card) {
    return page.evaluate(({ card, sel }) => {
      ...
      const sepClass = sel.viewedStatusLineSeparatorCharClass || '┬╖ΓÇó|';
